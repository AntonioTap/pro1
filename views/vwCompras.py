...
def gua_cambios_compra(id_compra, id_producto, fechaCompra, cantidad, precioCompra):
    conexion = conectar_bd()
    cursor = conexion.cursor()
...
def obt_lista_nombres_productos():
    conexion = conectar_bd()
    cursor = conexion.cursor()
...
def obt_id_producto_por_nombre(nombre_producto):
    conexion = conectar_bd()
    cursor = conexion.cursor()
...
def edi_compra_route(id):
    compra = obt_compra_por_id(id)
    ...
    if request.method == 'POST':
        ...
        gua_cambios_compra(id, id_producto, fechaCompra, cantidad, precioCompra)
        return redirect('/lista_compras')

    nombres_productos = obt_lista_nombres_productos()
    return render_template('editar_compra.html', compra=compra, nombres_productos=nombres_productos)

def eli_compra_por_id(id_compra):
    conexion = conectar_bd()
    cursor = conexion.cursor()
